...
While Supabase Auth verifies _who_ the user is, it must not dictate _what_ business operations the user can perform across CreatorConnect. Placing complex business authorization, brand agency tenancy, project participant checks, and escrow permissions into JWT claims creates bloated tokens, causes stale permission bugs, and violates separation of concerns.
